...
def cheak_attack(board: int, row: int, column: int) -> bool:
...
def queens(board: int, row = 0) -> bool:
    ...
    for column in range(n):
        if not cheak_attack(board, row, column):
            board[row][column] = 1
            success, final_board = queens(board, row + 1)
            if success:
...
if result:
    print(f'No, queens dont beat other = {result}')
else:
    print(f'Yes, queens beat each other = {result}')
